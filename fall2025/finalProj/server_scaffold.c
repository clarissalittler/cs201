 ...
 * 1. Rewind file to beginning: rewind(f);
 * 2. Truncate file to clear old contents: ftruncate(fileno(f), 0);
 * 3. Write all lines back to file using fprintf(f, "%s", ls[i]);
 * 4. Free memory for each line: free(ls[i]); (inside loop)
 * 5. Close the file: fclose(f);
 *
 * Note: This function frees individual lines but NOT the ls array itself!
 ...
 * TODO: Implement this function to:
 * 1. Allocate the lines array: d->lines = malloc(MAX_LINES * sizeof(char*));
 * 2. Initialize the line count: d->numLines = 0;
 * 3. Read lines from the file one by one using fgets()
 * 4. For each line: allocate memory with malloc(strlen(line) + 1)
 * 5. Copy the line and store in d->lines[d->numLines]
 * 6. Increment d->numLines
...
void* threadHandler(void* arg) {
    ...
    // Show a menu with options (View, Insert, Delete, Edit, Exit)
    // Get user's choice and call the appropriate function
    // Continue until user chooses to exit
    // Don't forget to clean up all allocated resources!

    // TODO: Clean up resources before exiting
    // Remember to:
    // 1. Send goodbye message: write(d->sockfd, "Goodbye!\n", 9);
    // 2. Save and free file: cleanUp(ourFile, d->lines, d->numLines);
    // 3. Free the lines array itself: free(d->lines);
    // 4. Free the filename: free(filename);
    // 5. Close socket: close(d->sockfd);
    // 6. Free client data: free(d);
