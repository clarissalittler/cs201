...
//
// The C version was a swarm of malloc/free, manual array shifts,
// fileno/ftruncate/fseek dance, and a fgets-vs-scanf gotcha. The
// Rust version is mostly Vec<String> and standard iterator methods,
// plus File::create to truncate. No raw pointers, no manual free,
// no chance of leaking a line buffer.
...
fn main() -> io::Result<()> {
    ...
    // slurp the whole file into a Vec<String> -- one line per element
    let lines: Vec<String> = BufReader::new(File::open(&path)?)
        .lines()
        .collect::<io::Result<Vec<_>>>()?;

    let mut lines = lines; // shadow as mutable
...
fn edit(lines: &mut Vec<String>) -> io::Result<()> {
    ...
    if i >= lines.len() { println!("out of range"); return Ok(()); }
...
fn insert(lines: &mut Vec<String>) -> io::Result<()> {
    ...
    lines.insert(i, s);  // Vec::insert handles the shift for us
    Ok(())
}
...
fn delete(lines: &mut Vec<String>) -> io::Result<()> {
    ...
    if i >= lines.len() { println!("out of range"); return Ok(()); }
    lines.remove(i);     // Vec::remove handles the shift AND frees the String
    Ok(())
}
