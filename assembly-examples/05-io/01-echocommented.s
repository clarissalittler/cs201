...
_start:
    ...
    # - %rcx and %r11 are clobbered (as noted at the top of this file);
    #   everything else, including %rdi/%rsi/%rdx, is preserved by the kernel
    # - Still, it's a good habit not to lean on argument registers
    #   surviving: the moment you wrap this in a function and *call* things,
    #   caller-saved registers really can change out from under you

    ## now I'm back, and the return value is in rax
...
//
// Assemble and link:
//   as 01-echocommented.s -o echo.o
//   ld echo.o -o echo
//
// Run interactively:
//   ./echo
...
//   lea buffer(%rip), %rsi   # RIP-relative addressing can't take an index
//                            # register, so get the base address first...
//   mov $0, %rcx             # Index counter
//   uppercase_loop:
//       cmp %rbx, %rcx   # Done processing?
//       jge write_out
//       movb (%rsi, %rcx, 1), %al   # ...then index off %rsi
//       cmp $'a', %al
//       jl skip
//       cmp $'z', %al
//       jg skip
...
// as 01-echocommented.s -o 01-echo.o
...
//
// 3. End of input (Ctrl+D sends EOF):
//    $ ./01-echo
//    [Ctrl+D]
