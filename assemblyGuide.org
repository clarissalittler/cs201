...
** Why learn it?
Okay and obvious question is "why bother learning assembly?". I mean, we have compilers, right? What's the actual thing that it does for us?
...
** Registers: what are they and why do we need them?
 So we're going to spend a lot of time talking about /registers/. All processors have some form of registers. These are the most low-level storage space, which act basically like "scratch paper" for the processor: a place where computations are done quickly and results are thrown away when no longer needed. Memory is /slow/, and we'll talk more about why later in the course, but the reality is that a register can be accessed in a /billionth/ of a second while memory might be 1000x or 10000x slower than that. So you need to use registers as much as possible. In fact, while some instructions let you load or save data directly to memory you generally can't load from a location in memory, do an operation, then save to a location in memory as a single step. You have to use at least one register as an intermediary storage space, a kind of workspace, before finalizing your work. So while having more registers would be good, the problem, though, is that each register complicates the physical hardware of the chip.

 For example, on x86-64 systems, you have /16/ (ordinary) registers[fn:2]. Each of which can hold 64bits (8 bytes) of data. That's not much! 
...
** Register reference
 ...
 For floating-point and vector/SIMD registers, see the reference section at the end of this document.
...
We can summarize this in the following table where we assume that we've just done =cmp a,b= (I always double-check this table when I need it, and you should too)
...
| jge | jump if b greater than or equal to a  |
